...
3. **Enter all Unity path fields** — Fill in the Editor path, source project path, sync project path, and build output path. You can use environment variables (e.g., `%USERPROFILE%`) or absolute paths. CADET will validate these paths exist when you save.
4. **Set the build executable name** — Enter the exact name the Unity build should produce (e.g., `MyGame.exe`).
5. **Select the target OS** — Choose Windows, macOS, or Linux depending on what platform you are building for. This affects build settings and validation logic.
...
- **Repository URL** — The Git repository URL (HTTPS or SSH). CADET will clone or pull from this repo before each build.
